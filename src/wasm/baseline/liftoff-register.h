...
class LiftoffRegister {
 ...
 public:
  explicit LiftoffRegister(Register reg) : LiftoffRegister(reg.code()) {
    std::cout << reg << " " << reg.code() << std::endl;
  ...
  explicit LiftoffRegister(Simd128Register reg)
    ...
    std::cout << " LiftoffRegister "<< kAfterMaxLiftoffFpRegCode << " " << reg.code() << std::endl;
    std::cout << "code_ "<<  (int64_t)code_ << std::endl;
...
class LiftoffRegList {
 public:
  ...
  LiftoffRegister set(LiftoffRegister reg) {
    ...
    std::cout << "set " << (storage_t{1} << reg.liftoff_code()) << std::endl;
    return reg;
  }
